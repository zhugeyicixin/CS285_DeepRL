...
class FFModel(nn.Module, BaseModel):
    ...
    def forward(
        ...
        if not isinstance(obs_unnormalized, torch.Tensor):
            obs_unnormalized = ptu.from_numpy(obs_unnormalized)

        if not isinstance(acs_unnormalized, torch.Tensor):
            acs_unnormalized = ptu.from_numpy(acs_unnormalized)

        if not isinstance(obs_mean, torch.Tensor):
            obs_mean = ptu.from_numpy(obs_mean)

        if not isinstance(obs_std, torch.Tensor):
            obs_std = ptu.from_numpy(obs_std)

        if not isinstance(acs_mean, torch.Tensor):
            acs_mean = ptu.from_numpy(acs_mean)

        if not isinstance(acs_std, torch.Tensor):
        ...
        obs_normalized = normalize(data=obs_unnormalized, mean=obs_mean, std=obs_std)  # TODO(Q1)
        acs_normalized = normalize(data=acs_unnormalized, mean=acs_mean, std=acs_std)  # TODO(Q1)
        ...
        next_obs_pred = obs_unnormalized + unnormalize(
            data=delta_pred_normalized, mean=delta_mean, std=delta_std
        )  # TODO(Q1)
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        """
        prediction = ptu.to_numpy(self.forward(
